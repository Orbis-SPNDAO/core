    ...
    Ownable
{
    ...
    function safeMint(address to, string memory uri) public {
        ...
        _safeMint(to, tokenId); // safeMint is not currently supported by on FVM
    ...
    function userBurn(uint256 id) public {
    ...
    {
        uint256[] memory ownersTokens = ownerToTokenIds[msg.sender];
        uint indexToBeDeleted;
        uint length = ownersTokens.length;
        for (uint256 i = 0; i < length; i++) {
            if (ownersTokens[i] == tokenId) {
                indexToBeDeleted = i;
                break;
            }
        }
        ownersTokens[indexToBeDeleted] = ownersTokens[length - 1];
        ownerToTokenIds[msg.sender] = ownersTokens;
